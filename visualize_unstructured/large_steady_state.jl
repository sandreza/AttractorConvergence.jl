...
self_error = zeros(25, 5)
self_relative_error = zeros(25, 5)
time_mean = read(t_hfile["z cumulants"])
for i in reverse(1:25)
    ...
    self_error[i, :] = abs.(ensemble_mean[i, :] .- ensemble_mean[end, :])
    self_relative_error[i, :] = self_error[i, :] ./ abs.(ensemble_mean[end, :])
end
close(e_hfile)
close(t_hfile)
...
colors = [:red, :purple, :blue, :orange, :green]
for i in 1:4
    y = log10.(self_relative_error[inds, i])
    scatter!(ax, x, y, color = (colors[i], 0.5), markersize = ms, label = "cumulant $i")
end
lines!(ax, x, -0.0 .- x, color = (:black, 0.5), linestyle=:dash, linewidth = lw)
axislegend(ax, position=:rt, framecolor=(:grey, 0.5), patchsize=(ls, ls), markersize=ms, labelsize=ls)
display(fig)
save(pwd() * figure_directory * "/cumulants_self_relative_error.png", fig)
...
error_list = zeros(25, 5)
relative_error = zeros(25, 5)
time_mean = read(t_hfile["z moments"])
for i in reverse(1:25)
    ensemble_mean[i, :] .= read(e_hfile["z moments $i"])
    partition_number[i] = read(e_hfile["number of partitions $i"])
    error_list[i, :] .= abs.(ensemble_mean[i, :] .- time_mean)
    relative_error[i, :] = error_list[i, :] ./ abs.(time_mean)
    self_error[i, :] = abs.(ensemble_mean[i, :] .- ensemble_mean[end, :])
...
for i in 1:4
    y = log10.(relative_error[inds, i])
    scatter!(ax, x, y, color = (colors[i], 0.5), markersize = ms, label = "moment $i")
end
lines!(ax, x, -0.0 .- x, color = (:black, 0.5), linestyle=:dash, linewidth = lw)
axislegend(ax, position=:rt, framecolor=(:grey, 0.5), patchsize=(ls, ls), markersize=ms, labelsize=ls)
display(fig)
save(pwd() * figure_directory * "/moments_relative_error.png", fig)
...
axislegend(ax, position=:rt, framecolor=(:grey, 0.5), patchsize=(ls, ls), markersize=ms, labelsize=ls)
display(fig)
save(pwd() * figure_directory * "/moments_self_relative_error.png", fig)